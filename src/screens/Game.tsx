...
export const Game: React.FC = () => {
  ...
  const handleClaim = async () => {
    if (BN.formatUnits(liquidity, 2).toNumber() === 0) {
      toast.error("Nothing to claim");
      return;
    }
    const tx = await contracts.gameContract.claimLiquidity(
      store.selectedBlockchainId
    );
    if (tx) {
    ...
    <Layout>
      ...
      <Footer>
        <TVLBlock>
          ...
          <span>
            <ButtonClaim onClick={handleClaim}>Claim liquidity</ButtonClaim>
          </span>
        </TVLBlock>
